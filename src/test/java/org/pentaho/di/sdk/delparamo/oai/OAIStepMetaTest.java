...
import org.pentaho.di.sdk.delparamo.oai.OAIStepMeta;
import org.pentaho.di.sdk.delparamo.oai.OAIStepData;
...
public class OAIStepMetaTest {
  ...
  public void testGetStepData() {
    OAIStepMeta m = new OAIStepMeta();
    assertEquals(OAIStepData.class, m.getStepData().getClass() );
  }
  ...
  public void testStepAnnotations() {
    ...
    Step stepAnnotation = OAIStepMeta.class.getAnnotation( Step.class );
    ...
    assertEquals(OAIStepMeta.class.getPackage().getName(), stepAnnotation.i18nPackageName() );
    ...
    InjectionSupported injectSupportAnnotation = OAIStepMeta.class.getAnnotation( InjectionSupported.class );
    ...
    for ( Field field : OAIStepMeta.class.getDeclaredFields() ) {
      if ( field.isAnnotationPresent( Injection.class ) || field.isAnnotationPresent( InjectionDeep.class ) ) {
        hasInjection = true;
      }
    }
    for ( Method method : OAIStepMeta.class.getDeclaredMethods() ) {
  ...
  public void testLoadSave() throws KettleException {
     ...
     */
    LoadSaveTester<OAIStepMeta> tester =
      new LoadSaveTester<OAIStepMeta>( OAIStepMeta.class, attributes );
  ...
  public void testChecks() {
    ...
    for ( CheckResultInterface result : checkResults ) {
      if ( result.getType() == CheckResultInterface.TYPE_RESULT_ERROR
          && result.getText().equals(BaseMessages.getString(OAIStepMeta.class, "OAI.CheckResult.ReceivingRows.ERROR" ) ) ) {
        foundMatch = true;
      }
    }
    ...
    for ( CheckResultInterface result : checkResults ) {
      if ( result.getType() == CheckResultInterface.TYPE_RESULT_OK
          && result.getText().equals(BaseMessages.getString(OAIStepMeta.class, "OAI.CheckResult.ReceivingRows.OK" ) ) ) {
        foundMatch = true;
      }
    }
    assertTrue( "The step checks should fail if no input fields are given", foundMatch );
