...
import org.pentaho.di.sdk.delparamo.csv2rdf.Mapper;
 ...
 *
 */
public class OAIStep extends BaseStep implements StepInterface {
   ...
   */
  public OAIStep(StepMeta s, StepDataInterface stepDataInterface, int c, TransMeta t, Trans dis) {
  ...
  public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
    ...
    OAIStepMeta meta = (OAIStepMeta) smi;
    OAIStepData data = (OAIStepData) sdi;
   ...
   * repeatedly by PDI for as long as it returns true. To indicate that a step
   * has finished processing rows this method must call setOutputDone() and
   * return false;
   *
   ...
   * call putRow() to pass the new row on. Above process may happen in a loop to
  ...
  public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
    ...
    if (first) {
      ...
      try {
        data.mapper = new Mapper(meta.getUri(), meta.getPrefix(), meta.getVerb(), meta.getContext(), data.outputRowMeta.getFieldNames(), meta.getMapping());
        data.mapper.init();
      } catch (Exception ex) {
      ...
      if (data.outputFieldIndex < 0) {
        log.logError(BaseMessages.getString(PKG, "OAIStep.Error.NoOutputField"));
        setErrors(1L);
        setOutputDone();
        return false;
      }
    ...
    try {
      ...
      data.mapper.map(outputRow);
      outputRow[data.outputFieldIndex] = data.mapper.getLastRDF();
    ...
    if (checkFeedback(getLinesRead())) {
      logBasic(BaseMessages.getString(PKG, "OAIStep.Linenr", getLinesRead())); // Some basic logging
    }
