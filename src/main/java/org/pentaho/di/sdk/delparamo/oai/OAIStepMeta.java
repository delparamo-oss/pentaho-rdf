...
@Step(
  id = "OAIStep",
  name = "OAIStep.Name",
  description = "OAIStep.TooltipDesc",
  image = "org/pentaho/di/sdk/delparamo/oai/resources/demo.svg",
  categoryDescription = "i18n:org.pentaho.di.trans.step:BaseStep.Category.Transform",
  i18nPackageName = "org.pentaho.di.sdk.delparamo.oai",
  documentationUrl = "OAIStep.DocumentationURL",
  casesUrl = "OAIStep.CasesURL",
  forumUrl = "OAIStep.ForumURL"
  )
@InjectionSupported( localizationPrefix = "OAIStepMeta.Injection." )
public class OAIStepMeta extends BaseStepMeta implements StepMetaInterface {
   ...
   */
  private static final Class<?> PKG = OAIStepMeta.class; // for i18n purposes
  ...
  public StepDialogInterface getDialog( Shell shell, StepMetaInterface meta, TransMeta transMeta, String name ) {
    return new OAIStepDialog( shell, meta, transMeta, name );
  }
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta transMeta,
      Trans disp ) {
    return new OAIStep( stepMeta, stepDataInterface, cnr, transMeta, disp );
  }
  ...
  public void setDefault() {
    ...
    setPrefix("oai_dc");
    setVerb("ListRecords");
    setUri("https://mock.com/foo");
    setMapping("CONSTRUCT { ?a ?b ?c . } WHERE { ?a ?b ?c . }");
    setContext("https://mock.foo/bar");
  }
  ...
  public String getXML() throws KettleValueException {
    ...
    xml.append( XMLHandler.addTagValue( "uri", uri ) );
    xml.append( XMLHandler.addTagValue( "prefix", prefix ) );
    xml.append( XMLHandler.addTagValue( "verb", verb ) );
    xml.append( XMLHandler.addTagValue( "mapping", mapping ) );
    xml.append( XMLHandler.addTagValue( "context", context ) );
    return xml.toString();
  }
  ...
  public void loadXML( Node stepnode, List<DatabaseMeta> databases, IMetaStore metaStore ) throws KettleXMLException {
    try {
      ...
      setUri( XMLHandler.getNodeValue( XMLHandler.getSubNode( stepnode, "uri" ) ) );
      setVerb( XMLHandler.getNodeValue( XMLHandler.getSubNode( stepnode, "verb" ) ) );
      setMapping( XMLHandler.getNodeValue( XMLHandler.getSubNode( stepnode, "mapping" ) ) );
      setContext( XMLHandler.getNodeValue( XMLHandler.getSubNode( stepnode, "context" ) ) );
    } catch ( Exception e ) {
  ...
  public void saveRep( Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step )
    ...
    try {
      ...
      rep.saveStepAttribute( id_transformation, id_step, "uri", uri ); //$NON-NLS-1$
      rep.saveStepAttribute( id_transformation, id_step, "prefix", prefix ); //$NON-NLS-1$
      rep.saveStepAttribute( id_transformation, id_step, "verb", verb ); //$NON-NLS-1$
      rep.saveStepAttribute( id_transformation, id_step, "mapping", mapping ); //$NON-NLS-1$
      rep.saveStepAttribute( id_transformation, id_step, "context", context ); //$NON-NLS-1$
    } catch ( Exception e ) {
  ...
  public void readRep( Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases )
    ...
    try {
      ...
      uri  = rep.getStepAttributeString( id_step, "uri" ); //$NON-NLS-1$
      prefix  = rep.getStepAttributeString( id_step, "prefix" ); //$NON-NLS-1$
      verb  = rep.getStepAttributeString( id_step, "verb" ); //$NON-NLS-1$
      mapping  = rep.getStepAttributeString( id_step, "mapping" ); //$NON-NLS-1$
      context  = rep.getStepAttributeString( id_step, "context" ); //$NON-NLS-1$
    } catch ( Exception e ) {
  ...
  public void check( List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev,
    ...
    if ( input != null && input.length > 0 ) {
      cr = new CheckResult( CheckResult.TYPE_RESULT_OK,
        BaseMessages.getString( PKG, "OAI.CheckResult.ReceivingRows.OK" ), stepMeta );
      remarks.add( cr );
    } else {
      cr = new CheckResult( CheckResult.TYPE_RESULT_ERROR,
        BaseMessages.getString( PKG, "OAI.CheckResult.ReceivingRows.ERROR" ), stepMeta );
      remarks.add( cr );
    }
  }
}
